...
def mostrar_tablero(juego):
	...
	for fil in range(len(juego.tablero)):
		for col in juego.tablero[fil]:
			...
			gamelib.draw_text(juego.tablero[indice_fil][indice_col], X_TABLERO + indice_col * (STEP_X_TARJETA + SEP_TARJETA) + X_TEXTO_TARJETA + X_TEXTO_TARJETA_INV, Y_TABLERO + indice_fil * (STEP_Y_TARJETA + SEP_TARJETA) + Y_TEXTO_TARJETA - Y_TEXTO_TARJETA_INV, fill = "brown", anchor = 'w', size = 7, angle = 180)
